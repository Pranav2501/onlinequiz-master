...
def take_exam_view(request,pk):
    ...
    questions=QMODEL.Question.objects.all().filter(course=course)
    instruction=QMODEL.Instructions.objects.all().filter(course=course)
    total_marks=0
    for q in questions:
        total_marks=total_marks + q.marks
    
    return render(request,'student/take_exam.html',{'course':course,'total_questions':total_questions,'total_marks':total_marks,'instruction':instruction})
